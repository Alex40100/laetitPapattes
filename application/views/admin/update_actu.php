      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="row">
              ...
              <?=  form_open_multipart('admin/update_actu', ['class' => 'form-horizontal']); ?>
              <div class="form-group row">
                <div class="col-md <?= empty(form_error('title'))?'':'has-error' ?>">
                    <?= form_input(['name' => "title", 'id' => "title",'placeholder' =>"Titre de l'actualité *",'class' => 'form-control'], set_value('title',$actu_item['title'])) ?>  
                    <span class="help-block text-danger"><?= form_error('title'); ?></span>
                </div>
              </div>
              <div class="form-group row">
                <div class="col-md <?= empty(form_error('content'))?'':'has-error' ?>">
                    <?= form_textarea(['name' => "content", 'id' => "content",'placeholder' =>"Contenu de l'actualité *", 'class' => 'form-control editor'], set_value('content',$actu_item['content'])) ?>    
                    <span class="help-block text-danger"><?= form_error('content'); ?></span>
                </div>
              </div>
              <div class="form-group ">
                    <?= form_submit("send", "Modifier", ['class'=>"btn btn-success"]); ?>
              </div>
              <?=  form_close(); ?>
          </div>
